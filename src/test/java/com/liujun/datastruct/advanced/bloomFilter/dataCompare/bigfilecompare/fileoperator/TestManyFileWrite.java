...
public class TestManyFileWrite {
  ...
  public void testFileWriteSize() {
    ...
    try (ManyFileWriteBase instance = ManyFileWrite.manyFileWriteBySize(path, fileName)) {
      instance.openFile();
  ...
  public void testFileWriteLine() {
    ...
    try (ManyFileWriteBase instance = ManyFileWrite.manyFileWriteByLine(path, fileName)) {
      instance.openFile();
      for (int i = 0; i < 1024; i++) {
        String dataLine = i + "-- this is test index,name;";
        instance.writeLine(dataLine);
