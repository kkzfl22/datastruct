...
public class Solution implements SolutionInf {
  ...
  public String minWindow(String s, String t) {
    ...
    Map<Integer, String> dataMatch = new HashMap<>(s.length(), 1);
    ...
    while (right < srcData.length) {
      ...
      if (targetMap.containsKey(item)) {
        ...
        while (slideMap(targetMap, slideMatch)) {
          dataMatch.put(right - left, new String(srcData, left, right - left));
    ...
    String target = dataMatch.get(minWindow);

    if (null == target || target.length() < t.length()) {
      return "";
    }
